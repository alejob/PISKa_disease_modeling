...
\title{Modeling Ebola outbreak with Agent-rule based Stochastic Simulation} 
...
\author
{Alejandro Bernardin,$^{1,2\ast}$ Ignacio Fuenzalida,$^{1}$ Jonathan Dushoff,$^{3,4}$ Tomas Perez-Acle$^{1,2}$\\
\\
\normalsize{$^{1}$Computational Biology Lab, DLab, Fundacion Ciencia \& Vida}\\
\normalsize{$^{2}$Centro Interdisciplinario de Neurociencias, CINV, Universidad de Valparaiso}\\
\normalsize{$^{3}$Department of Biology, McMaster University, Hamilton, ON, Canada}\\
\normalsize{$^{4}$Institute of Infectious Disease Research, McMaster University, Hamilton, ON, Canada}\\
\\
\normalsize{$^\ast$E-mail: abernardin@dlab.cl}
}
