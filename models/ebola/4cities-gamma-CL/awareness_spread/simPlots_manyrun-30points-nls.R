...
#head() is used to avoid last element in the list
simdir <- head(list.dirs(path=simdir_to_list,recursive=FALSE),-1)
...
for (num_sim in simdir){
    ...
    #Finding lambda
    #nls: non linear least square, fit non linear models
    # y ~ a*exp(x*b)
    #model <- lm( log( replace(cities$infected[2:35], cities$infected[2:35]==0,1)) ~ cities$time[2:35])
    dfCities <- data.frame(x = cities$time[1:35], y = replace(cities$infected[1:35], cities$infected[1:35]==0,1 ))
    model <- nls(y ~ a*exp(x*b), data = dfCities, start=c(a=10, b=0.01))
    lambda <- coef(model)[2]
    #this expoential is to get a mean of exponentials.
    expo <- expo + predict(model)
    #print("hola1")

    #Finding lambda by cities
    dfSantiago <- data.frame(x = Santiago$time[1:35], y = replace(Santiago$infected[1:35], Santiago$infected[1:35]==0,1 ))
    fitSantiago <- nls(y ~ a*exp(x*b), data = dfSantiago, start=c(a=10, b=0.001))
    lambdaSantiago <- coef(fitSantiago)[2]	
    expoSantiago <- expoSantiago + predict(fitSantiago)
    #print("hola2")    

    dfValparaiso<- data.frame(x = Valparaiso$time[1:35], y = replace(Valparaiso$infected[1:35], Valparaiso$infected[1:35]==0,1 ))
    fitValparaiso <- nls(y ~ a*exp(x*b), data = dfValparaiso, start=c(a=10, b=0.001))
    lambdaValparaiso <- coef(fitValparaiso)[2]
    expoValparaiso <- expoValparaiso + predict(fitValparaiso)
    #print("hola3")    

    dfRancagua<- data.frame(x = Rancagua$time[1:35], y = replace(Rancagua$infected[1:35], Rancagua$infected[1:35]==0,1 ))
    fitRancagua <- nls(y ~ a*exp(x*b), data = dfRancagua, start=c(a=10, b=0.001))
    lambdaRancagua <- coef(fitRancagua)[2]
    expoRancagua <- expoRancagua + predict(fitRancagua)
    #print("hola4")

    dfLa_Serena<- data.frame(x = La_Serena$time[1:35], y = replace(La_Serena$infected[1:35], La_Serena$infected[1:35]==0,1 ))    
    fitLa_Serena <- nls(y ~ a*exp(x*b), data = dfLa_Serena, start=c(a=10, b=0.001))
    lambdaLa_Serena <- coef(fitLa_Serena)[2]
    #print("hola5")
    expoLa_Serena <- expoLa_Serena + predict(fitLa_Serena)
...
lines(cities_sum $time[1:35], expo)
...
lines(Santiago_sum $time[1:35], expoSantiago)
...
lines(Valparaiso_sum $time[1:35], expoValparaiso)
...
lines(La_Serena_sum $time[1:35], expoLa_Serena)
...
lines(Rancagua_sum $time[1:35], expoRancagua)
...
mtext("cases",side=2,line=5, cex=1.5)


dev.off()

#STATE PLOTS
pdf("infected_all_cities-joint.pdf",7,7)
...
#plot(cities_sum $time, cities_sum $dead, ann=F, col="orange", ylim=c(1,127000), yaxt="n",log="y") #original
#plot(cities_sum $time, cities_sum $dead, ann=F, col="orange", yaxt="n",log="y")
plot(cities_sum $time, Santiago_sum $infected, ann=F, col="orange", yaxt="n")
points(cities_sum $time, Valparaiso_sum $infected, ann=F, col="black")
points(cities_sum $time, La_Serena_sum $infected, ann=F, col="red")
points(cities_sum $time, Rancagua_sum $infected, ann=F, col="blue")


eaxis(1)  # x-axis
eaxis(2,drop.1 = FALSE,las = 1 ) # (eje, colocar 1x, orientacion)

legend("topleft", inset=.05, title="State all cities",c("Santiago","Valparaiso","La_Serena","Rancagua"), fill=c("orange","black","red","blue"), horiz=FALSE, )
title(main="States", xlab="time[days]", ylab="",cex.lab = 1.5, cex.axis=2.0)
