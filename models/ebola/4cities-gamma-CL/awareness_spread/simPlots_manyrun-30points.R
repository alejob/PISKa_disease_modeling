...
for (num_sim in simdir){
    ...
    modelSantiago <- lm( log(replace(Santiago$infected[2:35], Santiago$infected[2:35]==0, 1)) ~ Santiago$time[2:35])
    lambdaSantiago <- modelSantiago$coefficients[2]
    expoSantiago <- expoSantiago + exp(predict(modelSantiago))
    
    modelValparaiso <- lm( log( replace(Valparaiso$infected[2:35], Valparaiso$infected[2:35]==0, 1)) ~ Valparaiso$time[2:35])
    lambdaValparaiso <- modelValparaiso$coefficients[2]
    expoValparaiso <- expoValparaiso + exp(predict(modelValparaiso))
    
    modelRancagua <- lm( log( replace(Rancagua$infected[2:35], Rancagua$infected[2:35]==0, 1)) ~ Rancagua$time[2:35])
    lambdaRancagua <- modelRancagua$coefficients[2]
    expoRancagua <- expoRancagua + exp(predict(modelRancagua))

    modelLa_Serena <- lm( log( replace(La_Serena$infected[2:35], La_Serena$infected[2:35]==0, 1)) ~ La_Serena$time[2:35])
...
lines(cities_sum $time[2:35], expo)
...
lines(Santiago_sum $time[2:35], expoSantiago)
...
lines(Valparaiso_sum $time[2:35], expoValparaiso)
...
lines(La_Serena_sum $time[2:35], expoLa_Serena)
...
lines(Rancagua_sum $time[2:35], expoRancagua)
