...
for (num_sim in simdir){
    ...
    # x ~ y
    model <- lm( log( replace(cities$infected[2:10], cities$infected[2:10]==0,1)) ~ cities$time[2:10])
    ...
    modelSantiago <- lm( log(replace(Santiago$infected[2:10], Santiago$infected[2:10]==0, 1)) ~ Santiago$time[2:10])
    lambdaSantiago <- modelSantiago$coefficients[2]
    expoSantiago <- expoSantiago + exp(predict(modelSantiago))
    
    modelValparaiso <- lm( log( replace(Valparaiso$infected[2:10], Valparaiso$infected[2:10]==0, 1)) ~ Valparaiso$time[2:10])
    lambdaValparaiso <- modelValparaiso$coefficients[2]
    expoValparaiso <- expoValparaiso + exp(predict(modelValparaiso))
    
    modelRancagua <- lm( log( replace(Rancagua$infected[2:10], Rancagua$infected[2:10]==0, 1)) ~ Rancagua$time[2:10])
    lambdaRancagua <- modelRancagua$coefficients[2]
    expoRancagua <- expoRancagua + exp(predict(modelRancagua))

    modelLa_Serena <- lm( log( replace(La_Serena$infected[2:10], La_Serena$infected[2:10]==0, 1)) ~ La_Serena$time[2:10])
...
lines(cities_sum $time[2:10], expo)
...
lines(Santiago_sum $time[2:10], expoSantiago)
...
lines(Valparaiso_sum $time[2:10], expoValparaiso)
...
lines(La_Serena_sum $time[2:10], expoLa_Serena)
...
lines(Rancagua_sum $time[2:10], expoRancagua)
